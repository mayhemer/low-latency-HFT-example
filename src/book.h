...
private:
    ...
    struct PriceRec
    {
        ...
        uint64_t first_order{0}; // to clear it on TRADE
    ...
    struct InstrBook
    {
        InstrBook() : orders_buy(1024), orders_sell(1024) {}

        // order_id -> OrderRec
        std::unordered_map<uint64_t, OrderRec> orders_buy;
        std::unordered_map<uint64_t, OrderRec> orders_sell;

        // price -> PriceRec, best: bid = price_buy.crbegin(), ask = price_sell.cbegin(), both O(1)
